...
# 2. Download latest DepotDownloaderMod release
echo "Fetching latest DepotDownloaderMod release..."
LATEST_URL=$(curl -s "$DEPOTDOWNLOADER_URL_API" | grep "browser_download_url" | grep -i DepotDownloaderMod | cut -d '"' -f 4)

if [ -z "$LATEST_URL" ]; then
    echo "Failed to find DepotDownloaderMod download URL"
    exit 1
fi
